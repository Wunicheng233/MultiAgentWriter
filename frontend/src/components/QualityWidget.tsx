...
const getScoreColor = (score: number): string => {
  ...
  if (score >= 6) return 'text-terracotta'
  return 'text-red-500'
}
        ...
        <>
          ...
          {showDetails && dimensions.length > 0 && (
            <div className="mt-5 space-y-3">
              <p className="text-sm font-medium text-secondary">各维度评分</p>
              {dimensions.map((dim) => (
          ...
          <div className="mt-4 pt-4 border-t border-border">
            <div className="grid grid-cols-2 gap-4 text-center">
              <div>
                ...
                <p className="text-xs text-secondary">已评审章节</p>
              </div>
              <div>
                <p className="text-2xl font-semibold text-sage">
                  {(overallScore / 10 * 100).toFixed(0)}%
                </p>
                <p className="text-xs text-secondary">质量达标率</p>
              </div>
            </div>
          </div>
        </>
      ) : (
        <div className="rounded-lg border border-dashed border-border p-4 text-center text-secondary">
          <p className="text-sm">
